                    ...
                    for (let user of participants) {
                       let pp = 'https://telegra.ph/file/ddd756ee15579bdfba651.jpg'
                        try {
                            ...
                            text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome @user Jangan Lupa Baca Deskripsi Sebelum Bertindak').replace('@subject', groupMetadata.subject).replace('@desc', groupMetadata.desc.toString()) :(chat.sBye || this.bye || conn.bye || 'Tetap Putus Asa Jangan Semangat Dan Jadilah Beban Keluarga 🤙🗿')).replace('@user', '@' + user.split('@')[0])this.sendButtonImg(id, pp, text, "ᯤ by DaniBotz", "AWOKAWOAK", "bebanjoin", null)
                                }
                    }
                }
                break
